...
class scoreSheet():
    ...
    def drawSheet(self, score=0):
        ...
        # text = self.bigFont.render('Angle: ' + str(angle), 1, grey)
        # self.win.blit(text, (800, 330))
...
def redrawWindow(ball, line, shoot=False, update=True):
    ...
    win.blit(text, (18, 45))
    text = parFont.render(
        'Angle: ' + str(round(findAngle(pos)*180/math.pi, 2)) + " °", 1, (64, 64, 64))
    win.blit(text, (200, 10))
    text = parFont.render(
        'Power: ' + str(round(physics.findPower(power, findAngle(pos), time), 2)) + " m/s²", 1, (64, 64, 64))
    win.blit(text, (200, 45))
    ...
    pygame.draw.circle(win, (0, 0, 0), ball, 8)
    pygame.draw.circle(win, ballColor, ball, 7)
    # ubah
    for i in trajectory:
        x_path, y_path = i
        pygame.draw.circle(win, (0, 0, 0), (int(x_path), int(y_path)), int(2))
...
while True:
    ...
    while shoot:  # If we are shooting the ball
        ...
        if not(overHole(ballStationary[0], ballStationary[1])):
            ...
            # ubah
            x_path, y_path = ballCords

            trajectory.append(ballCords)
            # print(x, ' ', y)
