...
def reduce_noise_centroid_s(y, sr):
    ...
    threshold_l = 100  # np.min(cent)

    less_noise = AudioEffectsChain().lowshelf(gain=-12.0, frequency=threshold_l,
                                              slope=0.5).highshelf(gain=-12.0, frequency=threshold_h, slope=0.5).limiter(gain=6.0)
...
def reduce_noise_centroid_mb(y, sr):
    ...
    less_noise = AudioEffectsChain().lowshelf(gain=-30.0, frequency=threshold_l,
                                              slope=0.5).highshelf(gain=-30.0, frequency=threshold_h, slope=0.5).limiter(gain=10.0)
...
def reduce_noise_mfcc_down(y, sr):
    ...
    speech_booster = AudioEffectsChain().highshelf(frequency=min_hz*(-1)*1.2,
                                                   gain=-12.0, slope=0.6).limiter(gain=8.0)
...
def enhance(y):
    apply_audio_effects = AudioEffectsChain().lowshelf(gain=10.0, frequency=260, slope=0.1).reverb(reverberance=25,
                                                                                                   hf_damping=5, room_scale=5, stereo_depth=50, pre_delay=20, wet_gain=0, wet_only=False)  # .normalize()
...
def output_file(destination, filename, y, sr, ext=""):
    ...
    sf.write(destination, y, sr)  # 48000, 'PCM_24')
...
sample files:
    ...
    06_office.m4a
------------------------------------'''
samples = ['01_counting.wav', '02_wind_and_cars.wav',
           '03_truck.wav', '04_voices.wav', '05_ambeint.wav', '06_office.wav']
...
for s in samples:
    ...
    output_file('01_samples_trimmed_noise_reduced/',
                filename, y_reduced_power, sr, '_pwr')
    output_file('01_samples_trimmed_noise_reduced/',
                filename, y_reduced_centroid_s, sr, '_ctr_s')
    output_file('01_samples_trimmed_noise_reduced/', filename,
                y_reduced_centroid_mb, sr, '_ctr_mb')
    output_file('01_samples_trimmed_noise_reduced/',
                filename, y_reduced_mfcc_up, sr, '_mfcc_up')
    output_file('01_samples_trimmed_noise_reduced/', filename,
                y_reduced_mfcc_down, sr, '_mfcc_down')
    output_file('01_samples_trimmed_noise_reduced/',
                filename, y_reduced_median, sr, '_median')
